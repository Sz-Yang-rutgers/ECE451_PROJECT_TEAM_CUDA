...
// Branchless sphere structure (no is_sun)
struct Sphere {
    float x, y, z;
    float radius;
    float r, g, b;
    float lambert;   // 1.0 for normal shading, 0.0 for sun
    float emissive;  // >0 for sun emissive, 0 for planets
};
                              ...
                              float3_ cam_o, float3_ cam_target, float fov_scale)
{
   ...
   // 3. Shading (branchless for sun vs planets)
...
if (hit_idx != -1) {
    ...
    float lightTerm = s.lambert * diff + ambient;
    float brightness = lightTerm + s.emissive;
...
void load_data(const std::string& filename) {
    ...
    while (std::getline(infile, line)) {
        if (line.find("t=:") != std::string::npos) {
            ...
            sun.radius   = 0.0f;   // will be filled in main()
            sun.r = sun.g = sun.b = 0.0f;
            sun.lambert  = 0.0f;   // will be filled in main()
            sun.emissive = 0.0f;
        ...
        float x, y, z;
        if (ss >> name >> x >> y >> z) {
            // Ignore Sun lines from file – we already have our own Sun at index 0
            if (name == "Sun")
                continue;

            Sphere s;
            s.x = x; s.y = y; s.z = z;
            s.radius   = 0.0f;  // set later
            s.r = s.g = s.b = 0.0f;
            s.lambert  = 0.0f;
            s.emissive = 0.0f;
...
int main(int argc, char** argv) {
    ...
    for (int i = 0; i < (int)all_frames.size(); ++i) {
        ...
        for (int j = 0; j < (int)frame.size(); ++j) {
            ...
            if (j == 0) {
                // j == 0 is ALWAYS Sun (we created it in load_data)
                s.radius = max_r * 0.05f;
                s.r = 1.0f; s.g = 0.8f; s.b = 0.0f;

                s.lambert  = 0.0f;   // no Lambert shading
                s.emissive = 5.0f;   // strong emission term
            } else {
                // Planets
                s.radius = max_r * 0.008f;
                s.r = 0.0f; s.g = 1.0f; s.b = 1.0f;

                s.lambert  = 1.0f; // normal lambert shading
                s.emissive = 0.0f;
            }
        }
